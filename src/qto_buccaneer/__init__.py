...
QTO Buccaneer - Quantity Takeoff tools for data-savvy BIM rebels.QTO Buccaneer - Quantity Takeoff tools for data-savvy BIM rebels.



A Python-powered toolkit for exploring, extracting, and calculating quantities from IFC models.A Python-powered toolkit for exploring, extracting, and calculating quantities from IFC models.

Built for architects, engineers, and construction pros who know their way around Excel more than Python.Built for architects, engineers, and construction pros who know their way around Excel more than Python.
...
__version__ = __version__- Calculate metrics per room or space

__author__ = "Simon Dilhas"- Benchmark different projects to spot trends and outliers

- Export results to Excel and other report formats used by your team

# Expose utils modules- Create beautiful reports with plans, making information visible and manageable

from .utils.ifc_loader import IfcLoader- Define metric logic using a user-friendly YAML config file — no need to write code

from .utils.qto_calculator import QtoCalculator- Enrich and clean up IFC files more easily than working directly with raw ifcopenshell

- Repair IFC Models based on rules

# Expose IFC Viewer functionality from colab_viewer subpackage- Build up project specific workflows and apply the same rules consecutive to the models
...
__all__ = [
    ...
    'visualize_ifc',- metrics: Core metrics calculation functionality

    'IfcLoader',- enrich: IFC model enrichment and data cleaning

    'QtoCalculator'- reports: Export and reporting utilities
