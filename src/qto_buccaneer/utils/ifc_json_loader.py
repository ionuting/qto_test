...
class IfcJsonLoader:
    ...
    def _process_elements(self) -> Dict[str, dict]:
        ...
        for element in data_list:
            # Verificăm dacă element este un dicționar
    ...
    def get_element_by_id(self, element_id: str) -> Optional[dict]:
        """Returnează elementul după ifc_global_id (sau None dacă nu există)."""
        return self.elements.get(element_id)

    def to_dataframe(self, ifc_type: str) -> pd.DataFrame:
        """Returnează un DataFrame cu elementele de tipul dat."""
        elements = [
            elem for elem in self.elements.values()
            if elem.get("IfcEntity") == ifc_type
        ]
        if not elements:
            return pd.DataFrame()
        # Aplatizează proprietățile pentru DataFrame
        records = []
        for elem in elements:
            record = {k: v for k, v in elem.items() if k != "properties"}
            # Aplatizează proprietățile
            if "properties" in elem:
                for pset, props in elem["properties"].items():
                    for prop, val in props.items():
                        record[f"{pset}.{prop}"] = val
            records.append(record)
        return pd.DataFrame(records)

    def get_property_value(self, element: dict, pset: str, prop: str) -> Optional[Any]:
        """Returnează valoarea unei proprietăți dintr-un element."""
        properties = element.get("properties", {})
        pset_dict = properties.get(pset, {})
        return pset_dict.get(prop)

    def get_first_element_by_type(self, ifc_type: str) -> Optional[dict]:
        """
        Returnează primul element de tipul dat sau None dacă nu există.
        """
        df = self.get_elements_by_type(ifc_type)
        if df.empty:
            return None
        return df.iloc[0]
