...
class IfcFilter:
    @staticmethod
    def filter_elements(df, filters, logic="AND"):
        # Filtrare simplă pentru test
...
class IfcLoader:
    ...
    def get_entity_metadata(self, element):
        """Returnează metadatele de bază pentru un element IFC."""
        return {
            "GlobalId": getattr(element, "GlobalId", None),
            "Name": getattr(element, "Name", None)
        }

    def get_entity_geometry(self, element):
        """Returnează geometria de bază pentru un element IFC."""
        return {
            "GlobalId": getattr(element, "GlobalId", None),
